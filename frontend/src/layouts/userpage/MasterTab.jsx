...
import { getUserMasterListAPI } from '../../api/user';

MasterTab.propTypes = {
	userName: PropTypes.string.isRequired,
};

function MasterTab({ userName }) {
	const [itemInfo, setItemInfo] = useState({
		communityList: [],
		hasMoreItems: true,
	});

	const getUserMasterList = async page => {
		const { data } = await getUserMasterListAPI('nickname', userName, page, 12);
		return data;
	};

	const loadItems = async page => {
		const data = await getUserMasterList(page);
		setItemInfo(prev => ({
			...prev,
